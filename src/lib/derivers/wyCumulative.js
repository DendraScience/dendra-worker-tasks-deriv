...
function toWYDateTime(time) {
  return DateTime.fromMillis(time, DATE_TIME_OPTS)
    .startOf('month')
    .minus({ months: 9 })
    .startOf('year')
    .plus({ months: 9 })
...
async function init(
  ...
  while (currentDateTime.toMillis() <= lastDateTime.toMillis()) {
    const nextDateTime = currentDateTime.plus({ year: 1 })
...
async function run(
  ...
  while (true) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      console.log('vSum, point', { vSum, point })
...
module.exports = ctx => {
  return async (opts, pointCb, dataCb) =>
    dataCb ? run(ctx, opts, pointCb, dataCb) : init(ctx, opts)
}
