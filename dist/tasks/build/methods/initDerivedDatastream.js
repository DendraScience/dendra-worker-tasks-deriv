...
async function initDerivedDatastream(req, ctx) {
  ...
  logger.info('Finding source datastreams', {
    query
  });
  ...
  logger.info('Initializing deriver', {
    derivationMethod
  });
  ...
  if (change) {
    ...
    logger.info('Patching derived datastream', {
      _id: datastream._id,
      query
    });
    patchRes = await datastreamService.patch(datastream._id, {
      $set: {
    ...
    logger.info('Deleting all measurement data', {
      queryOptions,
      queryStr
    });
    ...
    try {
      ...
      if (err.message.includes('database not found')) logger.warn(err.message);else throw err;
    }
  }
  /*
