...
async function deriveDatapoints(req, ctx) {
  ...
  if (!(database && derivationMethod && measurement)) throw new Error('Spec incomplete');
  /*
  ...
  const queryStr = `DELETE FROM ${measurement} WHERE time >= ${updateTime === undefined ? startTime : updateTime} AND time < ${untilTime}`;
